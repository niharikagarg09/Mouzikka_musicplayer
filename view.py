...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Tum Hi Ho...''')
    ...
    def setup_player(self):
        try:
            ...
            if self.player.get_db_status():
                messagebox.showinfo("succes" ,"connected succesfully to the db")
                ...
                self.loadFavorite.config(command=self.load_song_from_favourites)


            else:
                raise Exception("sorry you can't save songs to favourites")
        ...
        img = tk.PhotoImage(file="./icons/icons/icon_music1.png")
        self.top.iconphoto(self.top, img)
        self.top.protocol("WM_DELETE_WINDOW",self.close_window)
        self.isPlaying=False
        self.isPaused=False
        self.my_thread=None
        self.isThreadRunnning=False

    def close_window(self):
       # print("window closed")
       # sys.exit(0)
       result = messagebox.askyesno("app closed!!" ,"DO you want to quit?")
       if result:
           self.player.close_player()
           messagebox.showinfo("have a good day" ,"Thakyou for using the app")
           self.top.destroy()
           #sys.exit()  # this will also  destroy the window
    ...
    def play_song(self):
        ...
        try:
            ...
            if(self.isThreadRunnning==False):
                self.isThreadRunnning=True
            else:
                self.stop_song()
                time.sleep(1)
    ...
    def remove_song(self):
        ...
        except (NoSongSelectedError) as e:
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            if (len(fav_song_index_tuple)==0):
                raise NoSongSelectedError("please select a song for removing from favourites")
            song_name =self.playList.get(fav_song_index_tuple[0])
    ...
    def load_song_from_favourites(self):
        try:
           ...
           if result_str.find("no song")!=-1:
               messagebox.showinfo("favourite  empty" ,result_str)
               return
